...
import com.v2ray.ang.databinding.ActivityObservabilityBinding
...
class ObservabilityActivity : BaseActivity() {
            ...
            while (isActive) {
                ...
                if (totals != null) {
                    ...
                    if (previousTotals != null) {
                        val elapsedMs = (sampleElapsed - lastSampleElapsed).coerceAtLeast(1L)
                        ...
                        val rateUp = (deltaUp * 1000L) / elapsedMs
                        val rateDown = (deltaDown * 1000L) / elapsedMs
                        lastRateUp = rateUp
    ...
    private fun accumulateSample(sampleAt: Long, rateUp: Long, rateDown: Long) {
        ...
        if (sampleAt - bucketStartAt >= bucketDurationMs) {
            ...
            history.add(TrafficSample(bucketStartAt, avgUp, avgDown))
            trimHistory()
            persistHistory()
            bucketStartAt = sampleAt
            bucketUpSum = 0L
    ...
    private fun renderChart() {
        ...
        val rangeSamples = history.filter { it.timestamp >= rangeStart }
        if (rangeSamples.size <= 1) {
        ...
        binding.chartTraffic.setSeries(
            downsampled.map {
                com.v2ray.ang.ui.widget.TrafficLineChartView.TrafficPoint(
    ...
    private fun buildPointSubtitle(point: com.v2ray.ang.ui.widget.TrafficLineChartView.TrafficPoint): String {
